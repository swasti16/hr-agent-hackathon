...
from src.utils.latency_logger import timed_stage, log_latency
...
class HRAgent:
    ...
    def ask(self, query: str, history_str: str = "") -> dict:
        ...
        # ======== Step 1: Safety Shield ========
        with timed_stage("shield", timings):
        ...
        if not shield.is_safe:
            timings["total"] = round(sum(v for v in timings.values() if isinstance(v, (int, float))), 4)
            log_latency(query, timings)
            return {
                "intents": ["BLOCKED"],
                "answer": (
        ...
        with timed_stage("greeting_check", timings):
        ...
        with timed_stage("intent_classification", timings):
